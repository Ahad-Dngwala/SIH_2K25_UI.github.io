      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button 
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg font-medium transition-all duration-300 hover:bg-green-700 hover:scale-105"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat, index) => (
            <div key={index} className="group relative">
              ...
              <div className="relative bg-white/90 backdrop-blur-sm rounded-2xl p-6 border border-slate-200 hover:border-green-400/50 transition-all duration-300 hover:transform hover:scale-105 shadow-sm hover:shadow-lg">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              ...
              <div className="h-64 bg-slate-50 rounded-lg flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-slate-600">Performance chart would be integrated here</p>
                </div>
              </div>
            </div>
                ...
                {recentTransactions.slice(0, 3).map(transaction => (
                  <div key={transaction.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-slate-800">{transaction.type}</div>
                        <div className="text-sm text-slate-600">{transaction.project}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-semibold text-slate-800">{transaction.credits} credits</div>
                      <div className="text-sm text-slate-600">₹{transaction.amount.toLocaleString()}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {selectedTab === 'portfolio' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold text-slate-800">Your Portfolio</h3>
              <button className="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 rounded-lg font-medium transition-all duration-300 hover:bg-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {portfolioProjects.map(project => (
                <div key={project.id} className="group relative bg-white rounded-2xl overflow-hidden border border-slate-200 hover:border-green-400/50 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/25 shadow-sm">
                  <div className="relative">
                    ...
                    <div className="absolute top-4 right-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                        project.return.startsWith('+') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      }`}>
                        {project.return}
                      </span>
                    </div>
                  ...
                  <div className="p-6">
                    <h4 className="text-lg font-bold text-slate-800 mb-2">{project.name}</h4>
                    <p className="text-sm text-slate-600 mb-4">{project.organization}</p>
                    
                    <div className="space-y-2 mb-4">
                      <div className="flex justify-between">
                        <span className="text-sm text-slate-500">Credits:</span>
                        <span className="font-semibold">{project.credits.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-slate-500">Investment:</span>
                        <span className="font-semibold">₹{project.investment.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-slate-500">Current Value:</span>
                        <span className="font-semibold">₹{project.currentValue.toLocaleString()}</span>
                      </div>
                    </div>
                    
                    <button className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-medium transition-all duration-300 hover:shadow-lg hover:shadow-green-500/25 hover:scale-105">
              ...
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-sm font-semibold text-slate-600">Project</th>
                ...
                <tbody>
                  {recentTransactions.map(transaction => (
                    <tr key={transaction.id} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="px-6 py-4 text-sm font-semibold text-slate-800">{transaction.project}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{transaction.credits}</td>
                      <td className="px-6 py-4 text-sm font-semibold text-slate-800">₹{transaction.amount.toLocaleString()}</td>
          ...
          <div className="space-y-4">
            {notifications.map(notification => (
              <div 
                ...
                className={`p-6 bg-white rounded-2xl border border-slate-200 hover:border-green-400/50 transition-all duration-300 cursor-pointer ${
                  !notification.read ? 'ring-2 ring-green-100' : ''
                }`}
              >
